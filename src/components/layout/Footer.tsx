...
export function Footer() {
  ...
  // Assets for sliding banners — trimmed to 6 (was 9) since each is
  // duplicated to make the marquee loop seamless, so this is 12 images
  // per row rather than 18.
  const showcaseAssets = [
    ...PlaceHolderImages.filter(img => ['project-1', 'project-2', 'project-3', 'mobile-1', 'mobile-2', 'mobile-3'].includes(img.id)),
  ];
  const reversedAssets = [...showcaseAssets].reverse();

  return (
    <footer className="bg-background pt-16 lg:pt-24 pb-8 lg:pb-16 border-t border-foreground/5 relative overflow-hidden">

      {/* Sliding Showcase Banners — plain CSS marquee (see .animate-marquee-*
          in globals.css), not a JS animation loop, since this runs on
          every page for as long as the tab is open. */}
      <div className="w-full space-y-12 lg:space-y-16 mb-12 lg:mb-16">

        {/* Row 1 */}
        <div className="flex whitespace-nowrap overflow-hidden -mx-4">
          <div className="flex gap-4 lg:gap-8 px-4 animate-marquee-left" style={{ animationDuration: '40s' }}>
        ...
        <div className="flex whitespace-nowrap overflow-hidden -mx-4">
          <div className="flex gap-4 lg:gap-8 px-4 animate-marquee-right" style={{ animationDuration: '50s' }}>
            {[...reversedAssets, ...reversedAssets].map((asset, i) => (
              <div key={`bounce-${i}`} className="w-48 lg:w-72 aspect-[4/5] relative rounded-3xl overflow-hidden apple-glass border-black/5 dark:border-white/5">
                <Image src={asset.imageUrl} alt={asset.description} fill className="object-cover opacity-80" data-ai-hint={asset.imageHint} />
              </div>
            ))}
          </div>
        ...
        {/* Row 3 */}
        <div className="flex whitespace-nowrap overflow-hidden -mx-4">
          <div className="flex gap-4 lg:gap-8 px-4 animate-marquee-left" style={{ animationDuration: '60s' }}>
            {[...showcaseAssets, ...showcaseAssets].map((asset, i) => (
              <div key={`float-${i}`} className="w-56 lg:w-80 aspect-video relative rounded-3xl overflow-hidden apple-glass border-black/5 dark:border-white/5">
                <Image src={asset.imageUrl} alt={asset.description} fill className="object-cover opacity-80" data-ai-hint={asset.imageHint} />
              </div>
            ))}
          </div>
